...
//    string vis_path = dir.path + "fused.ply.vis"
//    getSensorMap(vis_path, data.sensor_map);
...
//    cout << "\nRead colmap points from " << dir.read_file << endl;
    cout << "\nRead colmap points from " << "fused.ply" << endl;

    const auto point_file = dir.path + "fused.ply";
    const auto vis_path = dir.path + "fused.ply.vis";
    const auto cam_file = dir.path + "sparse/images.bin";
    vector<vector<int>> vis_info;

    // read Binary PLY with normal and color
    Mesh_ply aMesh;
    Import_PLY(point_file.c_str(), &aMesh);

    // read the infos
//    loadVisibilityFile(vis_file, vis_info);
    fstream vis_file(vis_path, std::ios::in | std::ios::binary);
    const size_t vis_num_points = colmap::ReadBinaryLittleEndian<uint64_t>(&vis_file);
    CHECK_EQ(vis_num_points, aMesh.mVertices.size());
    cout << "\t-read visibility information" << endl;

    getSensorMap(cam_file, data.sensor_map);
    cout << "\t-read " << data.sensor_map.size() << " images from sparse/images.bin" << endl;

    for(int k=0; k < data.sensor_map.size(); k++){
        auto current = data.sensor_map.find(k+1);
        cout << current->first << " ";
        cout << current->second << endl;
    }

    // parse the read points
    for(int i = 0; i < aMesh.mVertices.size(); i++){
        ...
        // sensor
        vertex_info vinf;
        vinf.normal = Vector(aMesh.mNormals[i].x, aMesh.mNormals[i].y, aMesh.mNormals[i].z);
        // color
        ...
        // attach info to point
        int num_visible_images = colmap::ReadBinaryLittleEndian<uint32_t>(&vis_file);
        std::vector<std::pair<double,Point>> sensor_points;
        for (uint32_t i = 0; i < num_visible_images; ++i) {
            const int image_idx = colmap::ReadBinaryLittleEndian<uint32_t>(&vis_file);
            // image_idx from the fused.ply.vis and image/camera idx from the sensorMap simply are not the same
                // careful here, the colmap camera index starts at 1 not at 0!!!!
            Point sensor_point = data.sensor_map.find(image_idx)->second;
            Vector v1 = sensor_point - pt;
            Vector v2 = vinf.normal;
            double angle = std::acos( v1 * v2 / std::sqrt(v1.squared_length()*v2.squared_length()));
            if(CGAL::cross_product(v1,v2).z()<0)
                angle = 2*M_PI-angle;
        ...
        sort(sensor_points.begin(), sensor_points.end(), sortSensors);
        vinf.sensor_vec = sensor_points[0].second-pt;
        for(auto& sensor_position : sensor_points){
//            if(applyTrafo)
//                applyTransformation(sensor_position.second, transformation_matrix);
            vinf.sensor_positions.push_back(sensor_position.second);
    ...
    auto stop = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::seconds>(stop - start);
    cout << "\t-parsed " << data.points.size() << " points" << endl;
    cout << "\t-in " << duration.count() << "s" << endl;
}
...
//{
//    FILE * fp = fopen(pszVisFile.c_str(), "rb");
//    if (fp == NULL){
...
//    if (vis_num_points < 1){
//        printf("no visibilty information\n");
//        fclose(fp);
...
{
    FILE* fp = fopen(pszImageFile.c_str(), "rb");
    if (fp == NULL){
        printf("Can not open the file %s\n", pszImageFile.c_str());
        return false;
    }
...
//        std::cout << num_camera_index << " " << img_name << " " << proj_center << std::endl;
        std::cout << num_image_index << " " << img_name << " " << proj_center << std::endl;

//        image_proj_center[num_camera_index] = proj_center;
        image_proj_center[num_image_index] = proj_center;

//        image_proj_center.insert(map<int, Point>::value_type(num_camera_index, proj_center));
    }
    fclose(fp);
